...
Orion360 has a powerful animation framework that handles value editing based animations efficiently. The developer needs to select one of supported parameters to be animated, the value range, easing curve, and direction. Optionally a callback can be added to get notified when the animation is finished (and perhaps to clean up or start another animation or action). 
